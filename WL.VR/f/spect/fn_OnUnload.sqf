...
[_displayDialog] spawn {
		params ["_displayDialog"];
...
[_displayDialog] spawn {
		...
		f_cam_reinforcement_menuShown = false;
		{
			...
			_pos set [1,(_pos select 1) - 0.03];
			(_displayDialog displayCtrl _x) ctrlSetPosition _pos;
		} foreach f_cam_reinforcement_menuControls;
		{(_displayDialog displayCtrl _x) ctrlCommit 0.6; } foreach f_cam_reinforcement_menuControls;
		waitUntil {ctrlCommitted (_displayDialog displayCtrl (f_cam_reinforcement_menuControls select 0))};
